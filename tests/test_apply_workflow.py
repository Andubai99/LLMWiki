...
def test_apply_rejects_unknown_or_uncited_claims(capsys):
    ...
    mixed_patch = valid_source_patch(claim_ids=["clm_manual_1", "clm_manual_weak"])
    mixed_patch["content"] = mixed_patch["content"].replace(
        'claim_ids: ["clm_manual_1"]',
        'claim_ids: ["clm_manual_1", "clm_manual_weak"]',
    )
    write_manual_run(root, "run_mixed_weak_claim", mixed_patch, [valid_claim(), uncited_claim])
