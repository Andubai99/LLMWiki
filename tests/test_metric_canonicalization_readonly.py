...
def test_metric_canonicalize_does_not_write_workspace_files(capsys) -> None:
...
def test_metric_canonicalize_does_not_call_mutating_or_provider_surfaces(monkeypatch, capsys) -> None:
    root = workspace_with_canonicalization_cases()
    capsys.readouterr()

    def forbidden(*args, **kwargs):
        raise AssertionError("metric canonicalize must be read-only")
