...
def test_research_dry_run_status_and_synthesis_are_read_only(monkeypatch, capsys) -> None:
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError("read-only V5.0 command must not call LLM or write")

    monkeypatch.setattr("llmwiki.research.relationships.create_provider", forbidden)

    assert main(["research", "graph", "--root", str(root), "--dry-run", "--json"]) == 0
    json.loads(capsys.readouterr().out)
    assert main(["research", "graph-status", staged["relationship_run_id"], "--root", str(root), "--json"]) == 0
    json.loads(capsys.readouterr().out)
    assert main(["research", "synthesize", staged["relationship_run_id"], "--root", str(root), "--json"]) == 0
    json.loads(capsys.readouterr().out)

    assert snapshot_workspace(root) == before


def test_research_graph_commands_do_not_call_forbidden_surfaces(monkeypatch, capsys) -> None:
    root = workspace_with_metric_repair_cases()
    capsys.readouterr()

    def forbidden(*args, **kwargs):
        raise AssertionError("research graph must not call mutating/parser surfaces")
